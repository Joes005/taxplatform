...
import { useFinancialYears, useCustomers } from "@/hooks/useAccounting";
...
export default function SalesInvoiceFormPage() {
  ...
  const createMutation = useCreateSalesInvoice(companyId);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      toast({ title: "Sales invoice created", variant: "success" });
      navigate(`/accounting/sales-invoices/${invoice.id}`);
    } catch (err) {
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Totals shown here are for preview — the server recalculates and validates everything on save.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 space-y-1.5">
              ...
              <Input id="inv-pos" {...register("place_of_supply")} />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-2">
          <Button type="button" variant="outline" onClick={() => navigate("/accounting/sales-invoices")}>Cancel</Button>
