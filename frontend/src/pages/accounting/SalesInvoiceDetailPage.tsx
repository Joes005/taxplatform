...
import { useCancelSalesInvoice, usePostSalesInvoice, useSalesInvoice } from "@/hooks/useSalesInvoices";
...
export default function SalesInvoiceDetailPage() {
  ...
  const { data: invoice, isLoading } = useSalesInvoice(companyId, invoiceId);
  const postMutation = usePostSalesInvoice(companyId ?? "");
  const cancelMutation = useCancelSalesInvoice(companyId ?? "");
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <Link to="/accounting/sales-invoices" className="flex items-center gap-1 text-xs font-medium text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-3 w-3" /> Back to sales invoices
      </Link>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {invoice.status === "DRAFT" && (
            <PermissionGate permission="SALES_POST">
          ...
          {invoice.status !== "CANCELLED" && (
            <PermissionGate permission="SALES_CANCEL">
              ...
              {invoice.items.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-muted-foreground">
                    {formatMoney((parseFloat(item.cgst_amount) + parseFloat(item.sgst_amount) + parseFloat(item.igst_amount) + parseFloat(item.cess_amount)).toFixed(2))}
                  </TableCell>
      ...
      <Card>
        <CardContent className="space-y-2 pt-6 text-sm">
          ...
          <SummaryRow label="Round Off" value={formatMoney(invoice.round_off)} />
