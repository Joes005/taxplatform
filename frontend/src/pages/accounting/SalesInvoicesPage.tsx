...
export default function SalesInvoicesPage() {
  ...
  if (!activeCompany) return <EmptyCompanyState icon={FileText} />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">GST tax invoices issued to customers</p>
        </div>
        <PermissionGate permission="SALES_CREATE">
          <Button asChild>
            <Link to="/accounting/sales-invoices/new">
      ...
      <div className="rounded-lg border border-border bg-white">
        {isLoading ? (
          ...
          <Table>
            ...
            <TableBody>
              {data.items.map((inv) => (
                <TableRow key={inv.id}>
                  <TableCell>
                    <Link to={`/accounting/sales-invoices/${inv.id}`} className="font-medium text-primary hover:underline">
                      {inv.invoice_number}
                    </Link>
                  </TableCell>
          ...
          <EmptyTableState icon={FileText} title="No sales invoices yet" hint="Create your first invoice to get started." />
        )}
      </div>
