...
class CreditNoteItemRead(BaseModel):
...
class CreditNoteCreate(BaseModel):
    ...
    def check_party_matches_type(self) -> "CreditNoteCreate":
        if self.note_type == NoteType.SALES:
            if self.customer_id is None:
                raise ValueError("customer_id is required for a SALES credit note")
            self.vendor_id = None
        else:
            if self.vendor_id is None:
                raise ValueError("vendor_id is required for a PURCHASE credit note")
