...
class DebitNoteService:
    ...
    async def create(
        ...
        if payload.note_type == NoteType.SALES:
            ...
            if payload.reference_sales_invoice_id:
                ...
                if ref_inv.customer_id != payload.customer_id:
                    raise ValidationAppError(
                        "Debit note customer must match referenced invoice customer",
                        code="CUSTOMER_MISMATCH",
                    )
                if total_note_amount > ref_inv.grand_total:
                    raise ValidationAppError(
                        f"Debit note amount ({total_note_amount}) cannot exceed referenced invoice amount ({ref_inv.grand_total})",
            ...
            if payload.reference_purchase_invoice_id:
                ...
                if ref_inv.vendor_id != payload.vendor_id:
                    raise ValidationAppError(
                        "Debit note vendor must match referenced invoice vendor",
        ...
        await self.posting.post_debit_note(company_id, note, current_user, meta)
