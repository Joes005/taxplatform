...
class PurchaseInvoiceService:
    def __init__(self, db: AsyncSession) -> None:
        self.db = db
        self.repo = PurchaseInvoiceRepository(db)
    ...
    async def _validate_references(
        ...
        vendor = await self.vendors.get_by_id_for_company(vendor_id, company_id)
        if vendor is None:
    ...
    def _build_items(self, items_payload):
        line_inputs = [
            LineItemInput(
                quantity=i.quantity,
    ...
    async def create(
        ...
        _vendor, fy = await self._validate_references(
            company_id, payload.vendor_id, payload.financial_year_id
        )
        ...
        if existing is not None:
            await self.audit.log(
                ...
                description=f"Duplicate purchase invoice attempt: {payload.invoice_number}",
                ip_address=meta.ip_address,
                user_agent=meta.user_agent,
            )
            raise DuplicateResourceError(
                "An invoice with this number, date, and vendor already exists",
        ...
        await self.audit.log(
            ...
            description=f"Purchase invoice '{invoice.invoice_number}' created",
    ...
    async def get(self, company_id: uuid.UUID, invoice_id: uuid.UUID) -> PurchaseInvoice:
        invoice = await self.repo.get_by_id_for_company(invoice_id, company_id)
        if invoice is None:
            raise NotFoundError("Purchase invoice not found", code="PURCHASE_INVOICE_NOT_FOUND")
        return invoice
    ...
    async def update(
        ...
        if payload.vendor_id is not None:
            await self._validate_references(company_id, payload.vendor_id, invoice.financial_year_id)

        invoice_date = payload.invoice_date or invoice.invoice_date
        if payload.invoice_date is not None:
            fy = await self.financial_years.get_by_id_for_company(
                invoice.financial_year_id, company_id
            )
        ...
        await self.audit.log(
            ...
            resource_type="purchase_invoice",
            resource_id=str(invoice.id),
            description=f"Purchase invoice '{invoice.invoice_number}' updated",
            ip_address=meta.ip_address,
            user_agent=meta.user_agent,
        )
        return invoice
    ...
    async def post(
        ...
        await self.audit.log(
            ...
            resource_type="purchase_invoice",
            resource_id=str(invoice.id),
            description=f"Purchase invoice '{invoice.invoice_number}' posted",
            ip_address=meta.ip_address,
            user_agent=meta.user_agent,
        )
        return invoice
    ...
    async def cancel(
        ...
        await self.audit.log(
            ...
            resource_type="purchase_invoice",
            resource_id=str(invoice.id),
            description=f"Purchase invoice '{invoice.invoice_number}' cancelled",
            ip_address=meta.ip_address,
            user_agent=meta.user_agent,
        )
        return invoice
